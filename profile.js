...
function renderProjects(projects) {
  ...
  projectsList.innerHTML = projects.map(project => `<article class="project-card"><div class="project-card-head"><div><h2>${escapeText(project.project_name)}</h2><time>${new Date(project.created_at).toLocaleDateString(undefined,{year:'numeric',month:'long',day:'numeric'})}</time></div><span class="status-badge">${statusLabels[project.status] || project.status}</span></div><div class="project-services">${(project.services || []).map(service => `<span>${escapeText(service.name)} × ${service.quantity}</span>`).join('')}</div><div class="project-meta"><div><small>Format</small><strong>${escapeText(project.format || 'To be discussed')}</strong></div><div><small>Music</small><strong>${escapeText(project.preferred_music || 'To be discussed')}</strong></div><div><small>Estimate</small><strong>$${Number(project.estimated_total).toFixed(0)}</strong></div></div></article>`).join('');
}
